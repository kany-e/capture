...
final class LiveMemaAPIClientTests: XCTestCase {
    override func setUp() {
        super.setUp()
    ...
    func testStableErrorEnvelopePreservesStatusCodeCodeAndMessage() async throws {
        ...
        do {
        ...
        } catch let error as MemaAPIError {
            XCTAssertEqual(
                error,
                .http(
                    statusCode: 404,
            ...
            XCTFail("Expected MemaAPIError, got \(error)")
        }
    ...
    private func makeClient() -> LiveMemaAPIClient {
        ...
        return LiveMemaAPIClient(
            baseURL: URL(string: "https://mema.test")!,
            session: session
        )
    }
