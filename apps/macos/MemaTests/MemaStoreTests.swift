...
@MainActor
final class MemaStoreTests: XCTestCase {
    ...
    func testAccessibilitySelectionBoundsSourceApplicationWithoutChangingText() async {
        let source = String(
            repeating: "s",
            count: MemaStore.maximumSourceApplicationLength + 20
        )
        let service = AccessibilitySelectionServiceStub(
            result: .success(
                AccessibilitySelectionSnapshot(
        ...
        XCTAssertEqual(
            ...
            MemaStore.maximumSourceApplicationLength
        )
    }
        ...
        let fallbackService = SelectionClipboardFallbackServiceStub(
            result: .success(
                AccessibilitySelectionSnapshot(
                    text: String(
                        repeating: "x",
                        count: MemaStore.maximumSelectedTextLength + 1
                    ),
                    sourceApplication: "WeChat",
                    selectionBoundsInAXScreenCoordinates: nil,
                    captureMethod: .clipboardFallback
    ...
    func testClipboardFallbackPreferenceIsOptInAndPersistsExplicitChanges() {
        let suiteName = "MemaStoreTests.clipboardFallback.\(UUID().uuidString)"
        ...
        XCTAssertTrue(
            defaults.bool(forKey: MemaStore.selectionClipboardFallbackUserDefaultsKey)
        )
    }
    ...
    func testLocalScreenshotTextAtSourceLimitIsAcceptedWithoutTruncating() async {
        let exact = String(
            repeating: "x",
            count: MemaStore.maximumSelectedTextLength
        )
        let store = MemaStore(
            client: RecordingAPIClient(),
            clipboardService: ClipboardServiceStub(
        ...
        XCTAssertEqual(
            ...
            MemaStore.maximumSelectedTextLength
        )
        XCTAssertEqual(store.quickCaptureDraft?.userNote, "")
    }
    ...
    func testImageAnalysisDefaultPersistsWhileDraftChoiceCanBeOverridden() async {
        let suiteName = "MemaStoreTests.image-analysis.\(UUID().uuidString)"
        let defaults = UserDefaults(suiteName: suiteName)!
        ...
        XCTAssertTrue(defaults.bool(forKey: MemaStore.imageAnalysisUserDefaultsKey))
    ...
    func testClosingDuringAmbiguousImageSaveRetriesTheExactUpload() async throws {
        let client = RecordingAPIClient(
            failFirstCreate: true,
            createStatus: .ready,
            createDelayNanoseconds: 50_000_000
        ...
        XCTAssertTrue(prepared)
        store.screenshotNoteKind = .image
        store.quickCaptureDraft?.userNote = "Original image note"
        ...
        let firstSaved = await firstSave.value
        XCTAssertFalse(firstSaved)
        XCTAssertTrue(store.quickCaptureCanSubmit)
        let retried = await store.submitQuickCapture()
        XCTAssertTrue(retried)
        let requests = await client.allImageCreateRequests()
        XCTAssertEqual(requests.count, 2)
    ...
    func testPreparingClipboardCapturePreservesTextAndSourceApplication() {
        ...
        let store = MemaStore(client: client, clipboardService: service)
    ...
    func testPreparingClipboardCaptureBoundsDerivedSourceApplication() {
        ...
        let source = String(
            repeating: "s",
            count: MemaStore.maximumSourceApplicationLength + 1
        )
        let store = MemaStore(
            client: client,
            clipboardService: ClipboardServiceStub(
    ...
    func testOversizedSelectionIsNotSubmittedOrTruncated() async {
        ...
        let original = String(repeating: "a", count: MemaStore.maximumSelectedTextLength + 1)
    ...
    func testOversizedNoteIsNotSubmittedOrTruncated() async {
        let client = RecordingAPIClient()
        let original = String(repeating: "n", count: MemaStore.maximumUserNoteLength + 1)
        let store = MemaStore(
            client: client,
            clipboardService: ClipboardServiceStub(
                result: .failure(ClipboardCaptureError.noText)
    ...
    func testNoteAtContractLimitCanBeSubmitted() async throws {
        let client = RecordingAPIClient(createStatus: .ready)
        let note = String(repeating: "n", count: MemaStore.maximumUserNoteLength)
        let store = MemaStore(
            client: client,
            clipboardService: ClipboardServiceStub(
                result: .failure(ClipboardCaptureError.noText)
    ...
    func testSearchRejectsOversizedAndControlCharacterQueriesLocally() async {
        ...
        store.query = String(repeating: "q", count: MemaStore.maximumSearchQueryLength + 1)
    ...
    func testSearchAtContractLimitIsSent() async {
        ...
        store.query = String(repeating: "q", count: MemaStore.maximumSearchQueryLength)
    ...
    func testInvalidQueryClearsAnOlderSearchProgressState() async {
        ...
        store.query = String(
            repeating: "q",
            count: MemaStore.maximumSearchQueryLength + 1
        )
        await store.search(debounce: false)
    ...
    func testSortPreferenceRequestsBackendAndPersists() async {
        let suiteName = "MemaStoreTests.sort.\(UUID().uuidString)"
        let defaults = UserDefaults(suiteName: suiteName)!
        defer { defaults.removePersistentDomain(forName: suiteName) }
        let client = RecordingAPIClient()
        let store = MemaStore(
        ...
        XCTAssertEqual(
            defaults.string(forKey: MemaStore.captureSortOrderUserDefaultsKey),
...
private actor RecordingAPIClient: MemaAPIClient {
    ...
    private let listFailure: MemaAPIError?
    private let searchFailure: MemaAPIError?
    ...
    init(
        ...
        listFailure: MemaAPIError? = nil,
        searchFailure: MemaAPIError? = nil,
