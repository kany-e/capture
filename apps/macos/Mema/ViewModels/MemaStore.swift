...
@MainActor
final class MemaStore: ObservableObject {
    ...
    static let maximumSourceTitleLength = 500
    static let maximumSourceURLLength = 2_048
    ...
    @Published private(set) var attachmentImageErrors: Set<String> = []

    private let client: any MemaAPIClient
    ...
    convenience init(client: any MemaAPIClient = LiveMemaAPIClient()) {
    ...
    var quickCaptureCanSubmit: Bool {
        ...
        if draft.kind == .screenshot, screenshotNoteKind == .image {
            if screenshotPreviewData != nil {
                return true
            }
            if let attemptedQuickCaptureRequest,
               case .image = attemptedQuickCaptureRequest {
                return true
            }
            return false
        }
    ...
    func loadLibrary(
        ...
        do {
            ...
            if initial || libraryCaptures.isEmpty {
                loadState = .failed(error.memaUserMessage)
            } else if !silent {
                notice = AppNotice(
                    style: .error,
                    message: error.memaUserMessage,
                    scope: .connection,
    ...
    func search(debounce: Bool = true, silent: Bool = false) async {
        ...
        do {
            ...
            connectionState = connectedStatePreservingAIConfiguration()
            dismissNotice(scope: .connection)
        } catch let error as MemaAPIError where error.statusCode == 404 {
            guard generation == searchGeneration,
                  !Task.isCancelled,
                  submittedQuery == query.trimmingCharacters(in: .whitespacesAndNewlines) else {
                return
            ...
            searchError = error.memaUserMessage
            captures = []
            preserveSelection()
            updateConnectionState(after: error)
        }
    ...
    func refreshSelectedCapture() async {
        ...
        do {
            ...
            upsert(capture)
        } catch let error as MemaAPIError where error.code == "capture_not_found" {
    ...
    func prepareClipboardCapture() -> Bool {
        ...
        do {
            ...
            clearQuickCapture()
            quickCaptureError = error.memaUserMessage
            notice = AppNotice(
                style: .warning,
                message: error.memaUserMessage,
                scope: .clipboard
    ...
    func prepareScreenshotCapture() async -> Bool {
        ...
        do {
            ...
            clearQuickCapture()
            quickCaptureError = error.memaUserMessage
            notice = AppNotice(style: .warning, message: error.memaUserMessage)
            return false
        }
    }
    ...
    func submitQuickCapture() async -> Bool {
        ...
        guard draft.characterCount <= Self.maximumSelectedTextLength else {
            quickCaptureError = "The selection is \(draft.characterCount.formatted()) characters. Mema can save up to \(Self.maximumSelectedTextLength.formatted()) characters without losing the original text."
            return false
        }

        guard draft.noteCharacterCount <= Self.maximumUserNoteLength else {
            quickCaptureError = "The note is \(draft.noteCharacterCount.formatted()) characters. Mema can save notes up to \(Self.maximumUserNoteLength.formatted()) characters. Your draft has not been changed."
            return false
        }
        ...
        case .screenshot:
            if screenshotNoteKind == .image {
                ...
                if let attemptedQuickCaptureRequest,
                   case let .image(attemptedImageRequest) = attemptedQuickCaptureRequest,
                   screenshotPreviewData == nil {
                    guard attemptedImageRequest.metadata == metadata else {
                        quickCaptureError = "A previous save may already exist, so Mema must retry the original source and note. Cancel and capture again if you want to change the note."
                        return false
                    }
                    currentRequest = attemptedQuickCaptureRequest
                    break
                }
        ...
        do {
            ...
            case .processing:
                notice = AppNotice(
                    style: .information,
                    message: "Saved. Mema is processing this memory.",
    ...
    func retryEnrichment(id: String) async {
        do {
        ...
        } catch let error as MemaAPIError where error.code == "capture_already_processing" {
            notice = AppNotice(
                style: .information,
            ...
            beginPolling(captureID: id)
        } catch let error as MemaAPIError where error.code == "openai_not_configured" {
            notice = AppNotice(style: .warning, message: error.localizedDescription)
        } catch {
            guard !Self.isCancellation(error) else { return }
            updateConnectionState(after: error)
    ...
    func loadAttachmentImage(_ attachment: CaptureAttachment) async {
        ...
        attachmentImageErrors.remove(attachment.id)
        ...
        do {
            ...
            guard !Self.isCancellation(error) else { return }
            attachmentImageErrors.insert(attachment.id)
            updateConnectionState(after: error)
        }
    }
    ...
    func updateCapture(id: String, request: CaptureUpdateRequest) async -> Bool {
        ...
        do {
            ...
            guard !Self.isCancellation(error) else { return false }
            captureUpdateError = error.memaUserMessage
            updateConnectionState(after: error)
            notice = AppNotice(
