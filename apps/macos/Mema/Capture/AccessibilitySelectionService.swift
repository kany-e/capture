    ...
    var errorDescription: String? {
        ...
        case .permissionRequired:
            return NSLocalizedString(
                "Mema needs Accessibility access to read selected text. Open System "
                    + "Settings > Privacy & Security > Accessibility, enable Mema, then try again.",
                comment: "Accessibility permission is required for selection capture"
            )
        case .noFocusedApplication:
            return NSLocalizedString(
                "Mema could not find the focused application. Return to the app, select text, "
                    + "and try again.",
        ...
        case .currentApplication:
            return NSLocalizedString(
                ...
                comment: "Mema itself is the focused application during selection capture"
            )
        case .noFocusedElement:
            return NSLocalizedString(
                "Mema could not find the focused control. Select text in the app and try again.",
                comment: "No focused control was available for selection capture"
            )
        case .secureTextInput:
            return NSLocalizedString(
                "Mema does not read selections from password or secure text fields.",
                comment: "Selection capture is blocked for secure text fields"
            )
        case .selectionSafetyUnavailable:
            return NSLocalizedString(
                "Mema could not verify that this control is safe to copy, so it did not use "
        ...
        case .selectionTooLong:
            return NSLocalizedString(
                "The selected text is longer than 12,000 characters. Select a smaller passage "
                    + "and try again; Mema did not truncate or save it.",
                comment: "The Accessibility text selection exceeded the source limit"
            )
        case .clipboardFallbackUnavailable:
            return NSLocalizedString(
                "Mema could not safely complete the clipboard compatibility fallback. Check "
        ...
        case .clipboardChangedDuringFallback:
            return NSLocalizedString(
                "Mema detected competing clipboard activity and stopped without preparing a "
                    + "capture. Check your clipboard before continuing.",
                comment: "Another clipboard writer raced the selection fallback"
            )
        case .clipboardRestoreFailed:
            return NSLocalizedString(
                "Mema captured the temporary copy but could not restore the previous "
...
struct SystemAccessibilitySelectionBackend: AccessibilitySelectionBackend, Sendable {
    ...
    func isCurrentApplication(_ application: SystemAccessibilityElement) -> Bool {
        ...
        guard let memaBundleIdentifier = Bundle.main.bundleIdentifier else {
            return false
        }
        return NSRunningApplication(processIdentifier: processIdentifier)?.bundleIdentifier
            == memaBundleIdentifier
    }
