...
def test_attachment_path_must_not_contain_database(tmp_path: Path) -> None:
    with pytest.raises(ValidationError, match="must not contain"):
        Settings(
            _env_file=None,
            mema_database_path=tmp_path / "storage" / "mema.db",
            mema_attachments_path=tmp_path / "storage",
        )
...
def test_database_path_must_not_contain_attachment_directory(tmp_path: Path) -> None:
    with pytest.raises(ValidationError, match="must not contain"):
        Settings(
            _env_file=None,
            mema_database_path=tmp_path / "mema.db",
            mema_attachments_path=tmp_path / "mema.db" / "images",
        )
...
def test_broad_or_malformed_cors_origin_is_rejected(origins: str) -> None:
    with pytest.raises(ValidationError):
        Settings(_env_file=None, mema_cors_origins=origins)
