"""SQLite connection and ordered migration support for Mema."""
...
def database_connection(database_path: Path) -> Iterator[sqlite3.Connection]:
    ...
    for created_parent in reversed(missing_parents):
        os.chmod(created_parent, PRIVATE_DIRECTORY_MODE)
    ...
    try:
        ...
        if not database_path.is_file():
            raise OSError(f"Database path is not a regular file: {database_path}")
    else:
        os.close(descriptor)

    # Existing databases may predate the private-storage policy. Tighten the
    # file on every connection without changing a user-supplied parent folder.
