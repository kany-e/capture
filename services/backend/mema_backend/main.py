"""Minimal FastAPI application for the Mema localhost service."""
...
from mema_backend.api_errors import error_response
from mema_backend.api_models import (
...
from mema_backend.attachments import (
    AttachmentStorage,
    AttachmentStorageError,
    AttachmentValidationError,
)
from mema_backend.config import get_settings
from mema_backend.cors import ConfiguredCORSMiddleware
...
from mema_backend.embeddings import EmbeddingProvider, OpenAIEmbeddingProvider
...
from mema_backend.models import AttachmentRecord, CaptureRecord
from mema_backend.ocr import OCRFailure, OCRProvider, OpenAIOCRProvider
...
from mema_backend.search import HybridSearchService
...
async def lifespan(_: FastAPI):
    ...
    try:
        apply_migrations(settings.mema_database_path)
        if settings.mema_attachments_path is None:  # pragma: no cover - validated.
            raise MigrationError("Attachment storage path was not resolved")
        storage = AttachmentStorage(settings.mema_attachments_path)
...
app = FastAPI(title="Mema Backend", version="1.0.0", lifespan=lifespan)
app.add_middleware(
...
def get_repository() -> CaptureRepository:
    settings = get_settings()
    return CaptureRepository(settings.mema_database_path, initialize=False)
...
def get_attachment_storage() -> AttachmentStorage:
    ...
    return AttachmentStorage(settings.mema_attachments_path)
...
def enrich_capture(
    ...
    if repository.get(str(capture_id)) is None:
        return error_response(
...
def health(response: Response) -> HealthResponse:
    settings = get_settings()
    database = check_database(settings.mema_database_path)
    try:
